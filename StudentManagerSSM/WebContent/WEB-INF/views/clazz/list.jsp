	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'gid',title:'年级名',width:150, sortable: true,
 		        ...
 		        {field:'remark',title:'备注',width:300,
 		        	formatter:function(value,index,row){
 		        		for (var i = 0; i <= row; i++){
 		        			if(value == ""){
 		        				return "<font color='red'>暂无备注</fond>";
 		        			}
 		        		}
 		        		return value;
 		        	}	
	    ...
	    $("#addDialog").dialog({
	  	...
	  	$("#editDialog").dialog({
			...
			onBeforeOpen: function(){
				...
				$('#edit_gid').combobox('setValue', selectRow.gid);
	  	...
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('reload',{
	  			...
	  			gid: $("#search-grade").combobox('getValue')
	  		});
	  	});
	});
	</script>
	...
	<div id="toolbar">
		<c:if test="${userType == 1 }">
			...
			<div>
				...
				年   级：<select id="search-grade" style="width: 200px; height: 30px;" name="search-grade" class="easyui-combobox">
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable" cellpadding="8">
	    		<tr >
	    			<td>班级名:</td>
	    			<td>
	    				<input id="add_name" type="text" name="name"  class="easyui-textbox" style="width: 200px; height: 30px;" data-options="required:true, missingMessage:'请填写班级名'"  />
	    			</td>
	    		</tr>
	    		<tr >
	    			<td>所属年级:</td>
	    			<td>
	    				<select id="add_gid" name="gid"  class="easyui-combobox" style="width: 200px; height: 30px;">
	    					<c:forEach items="${gradeList }" var="grade">
	    						<option value="${grade.id }" >${grade.name }</option>
	    					</c:forEach>
	    				</select>
    	...
    	<form id="editForm" method="post">
	    	...
	    	<table id="editTable" border=0 cellpadding="8" >
	    		...
	    		<tr>
	    			<td>年级名：</td>
	    			<td>
	    				<select id="edit_gid" name="gid" class="easyui-combobox" tyle="width: 200px; height: 30px;" type="text" data-options="required:true, missingMessage:'请填写班级名'">
	    					<c:forEach items="${gradeList }" var="grade">
	    						<option value="${grade.id }">${grade.name }</option>
	    					</c:forEach>
	    				</select>
