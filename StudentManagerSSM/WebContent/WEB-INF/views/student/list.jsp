...
<head>
				...
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:'name',title:'姓名',width:150, sortable: true},
 		      	...
 		      	{field:'sex',title:'性别',width:150, sortable: true},
 		       	{field:'cid',title:'所属班级',width:150, sortable: true,
 		        	formatter:function(value,index,row){
	    ...
	    $("#addDialog").dialog({
	  	...
	  	$("#search-btn").click(function(){
	  		$('#dataList').datagrid('reload',{
	  			...
	  			cid:$("#search-clazz-id").combobox('getValue')
	  		});
	  	});
	  	//上传图片按钮
	...
	function uploaded(e){
		...
		if(data.type == "success"){
			...
			$("#add-photo-preview").attr("src",data.src);
		...
		<form id="photoForm" method="post" enctype="multipart/form-data" action="upload_photo" target="photo_target">
	    	<table id="addTable1" cellpadding="8">
	    		<tr >
	    			<td>预览头像:</td>
	    			<td>
	    				<img id="add-photo-preview" alt="照片" style="max-width: 100px; max-height: 100px;" title="照片" src="E:\TopSecret\java\.metadata\.plugins\org.eclipse.wst.server.core\tmp1\\wtpwebapps\StudentManagerSSM\upload\1587032776314.jpg" />
	    			</td>
	    		</tr>
	    		<tr >
	    			<td>学生头像:</td>
   		...
   		<form id="addForm" method="post">
	    	<table id="addTable2" cellpadding="8">
	    		...
	    		<tr >
	    			<td>学生学号:</td>
	    			<td>
	    				<input id="add_stuno"  class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="stuno" data-options="required:true, missingMessage:'请填写学生学号'"  />
	    			</td>
	    		</tr>
	    		<tr >
	    			<td>学生姓名:</td>
	    		...
	    		<tr >
	    			<td>所属班级:</td>
	    			<td>
	    				<select id="add_cid"  class="easyui-combobox" style="width: 200px;" name="cid" data-options="required:true, missingMessage:'请选择所属班级'">
	    					<c:forEach items="${clazzList}" var="clazz">
	    						<option value="${clazz.id }">${clazz.name }</option>
	    					</c:forEach>
	    				</select>
   		...
   		<form id="editForm" method="post">
	    	...
	    	<table id="editTable2" cellpadding="8">
	    		...
	    		<tr >
	    			<td>学生学号:</td>
	    			<td>
	    				<input id="edit_stuno"  class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="stuno" data-options="required:true, missingMessage:'请填写学生学号'"  />
	    			</td>
	    		</tr>
	    		<tr >
	    			<td>学生姓名:</td>
	    		...
	    		<tr >
	    			<td>所属班级:</td>
	    			<td>
	    				<select id="edit_cid"  class="easyui-combobox" style="width: 200px;" name="cid" data-options="required:true, missingMessage:'请选择所属班级'">
	    					<c:forEach items="${ clazzList}" var="clazz">
	    						<option value="${clazz.id }">${clazz.name }</option>
	    					</c:forEach>
	    				</select>
